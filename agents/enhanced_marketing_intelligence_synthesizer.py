...
# Try to import tactical coordinator
try:
    from agents.avatar_agnostic_coordinator import run_tactical_research_pipeline
    tactical_agent = run_tactical_research_pipeline
    TACTICAL_AGENT_AVAILABLE = True
    print("✅ Successfully imported tactical research pipeline")
except ImportError:
    print("⚠️ Tactical research pipeline not available")
    TACTICAL_AGENT_AVAILABLE = False
    tactical_agent = None

app = FastAPI(title="Market Research Agent Team", version="3.1.0")
...
async def root():
    return {
        ...
        "tactical_pipeline": TACTICAL_AGENT_AVAILABLE,
...
@app.post("/research/tactical-conversion")
async def tactical_conversion_research(context: SimpleBusinessContext):
    """
    Run enhanced tactical research pipeline: ICP + Interviews + Marketing + Conversion Copy
    Generates micro-testable assets and high-converting copy for immediate deployment
    """
    
    session_id = f"tactical_research_{len(research_sessions) + 1}"
    ...
    try:
        if not TACTICAL_AGENT_AVAILABLE:
            return {
                "session_id": session_id,
                "status": "error",
                "message": "Tactical research pipeline not available. Using comprehensive analysis instead.",
                "fallback_url": "/research/comprehensive-analysis"
            }
        
        print(f"🎯 Starting tactical conversion research pipeline...")
        
        # Run the tactical coordinator
        tactical_results = tactical_agent(context.comprehensive_context)
        
        # Store results
        research_sessions[session_id]["agent_results"]["tactical"] = tactical_results
        research_sessions[session_id]["status"] = "completed"
        
        # Save tactical report to disk
        try:
            os.makedirs("reports", exist_ok=True)
            report_filename = f"reports/{session_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}_tactical.json"
            ...
            report_data = {
                ...
                "status": "completed",
                "research_type": "tactical_conversion_pipeline"
            }
            ...
            research_sessions[session_id]["report_file"] = report_filename
            print(f"📄 Tactical report saved to {report_filename}")
            
        except Exception as e:
            print(f"⚠️ Failed to save tactical report: {str(e)}")
        
        return {
            "session_id": session_id,
            "status": "completed",
            "message": "Tactical conversion research pipeline completed",
            "phases_completed": {
                "icp_research": "✅ Deep customer psychology with Schwartz analysis",
                "interview_intelligence": "✅ Persona interviews with authentic language" if tactical_results.get("success") else "⚠️ Fallback used",
                "tactical_marketing": "✅ High-intent conversion strategy" if tactical_results.get("success") else "⚠️ Fallback used",
                "conversion_copy": "✅ Micro-testable assets generated" if tactical_results.get("success") else "⚠️ Fallback used"
            },
            "pipeline_status": tactical_results.get("success", False),
            "agents_used": tactical_results.get("agents_used", {}),
            "deliverables": tactical_results.get("deliverables", {}),
            "expected_performance": tactical_results.get("expected_results", {}),
            "micro_testing_ready": True,
            "deployment_readiness": {
                "tofu_assets": "Ready for $25-50 micro-tests",
                "mofu_mechanisms": "25%+ conversion rate targets",
                "bofu_copy": "25-40% close rate optimization",
                "roi_projection": "3-5x industry average performance"
            },
            "full_results": tactical_results,
            "full_results_url": f"/research/{session_id}/results"
        }
        ...
        return {
            "session_id": session_id,
            "status": "error",
            "message": f"Error in tactical conversion research: {str(e)}",
            "troubleshooting": "Check tactical agent configurations"
        }
...
@app.get("/test-tactical")
async def test_tactical_form():
    """
    Test form for the tactical conversion research pipeline
    """
    html_content = """
    <!DOCTYPE html>
    <html>
    <head>
        <title>🎯 Test Tactical Conversion Pipeline</title>
        <style>
            body { 
                font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
                max-width: 900px; 
    ...
    <body>
        <div class="container">
            <h1>🎯 Tactical Conversion Pipeline Test</h1>
            
            <div class="description">
                <strong>ULTIMATE CONVERSION PIPELINE:</strong> This runs ICP Research → Interview Intelligence → Tactical Marketing → Conversion Copy Generation.
                <br><br>
                <strong>⏱️ Processing Time:</strong> 8-12 minutes (4 phases + asset generation)
            </div>
            
            <div class="features">
                <strong>What You Get:</strong>
                <ul class="feature-list">
                    <li><strong>TOFU Micro-Tests:</strong> MintCRO-style headlines with $25-50 budgets, CTR targets, success criteria</li>
                    <li><strong>MOFU Mechanisms:</strong> Lead magnets, webinars, case studies targeting 25%+ conversion rates</li>
                    <li><strong>BOFU Copy:</strong> Sales pages, email sequences, offers optimized for 25-40% close rates</li>
                    <li><strong>Tactical Frameworks:</strong> Step-by-step implementation with success metrics</li>
                    <li><strong>High-Intent Strategy:</strong> Behavioral signals, premium positioning, conversion optimization</li>
                </ul>
            </div>
            
            <form id="tacticalForm">
                <textarea id="business_context" placeholder="Paste your comprehensive business context here (same format as other forms)..." required></textarea>
                
                <button type="submit">🎯 Generate Tactical Conversion Assets</button>
            </form>
        ...
        <script>
            document.getElementById('tacticalForm').onsubmit = async function(e) {
                e.preventDefault();
                
                const button = document.querySelector('button');
                const results = document.getElementById('results');
                
                button.textContent = '🎯 Generating Conversion Assets...';
                button.disabled = true;
                
                results.style.display = 'block';
                results.innerHTML = '<div style="text-align: center; padding: 40px; color: #dc2626; font-weight: 600;">🎯 Tactical Pipeline Working...<br><br>🧠 Phase 1: ICP Research<br>🎭 Phase 2: Interview Intelligence<br>📊 Phase 3: Tactical Marketing Strategy<br>✍️ Phase 4: Conversion Copy Generation<br><br>⏱️ Generating micro-testable assets...</div>';
                ...
                try {
                    const response = await fetch('/research/tactical-conversion', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify(data)
                    });
                    ...
                    const result = await response.json();
                    results.innerHTML = '<h3>🎉 Tactical Conversion Assets Generated!</h3><pre>' + JSON.stringify(result, null, 2) + '</pre>';
                } catch (error) {
                    results.innerHTML = '<h3>❌ Error:</h3><p>' + error.message + '</p>';
                }
                
                button.textContent = '🎯 Generate Tactical Conversion Assets';
                button.disabled = false;
            };
        </script>
    </body>
